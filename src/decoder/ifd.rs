...
impl Entry {
  ...
  pub fn val<R: Read + Seek>(
    &self, limits: &super::Limits, bigtiff: bool, reader: &mut SmartReader<R>,
  ) -> TiffResult<Value> {
    ...
    let tag_size = match self.type_ {
      ...
      Type::LONG8 | Type::SLONG8 | Type::DOUBLE | Type::RATIONAL | Type::SRATIONAL | Type::IFD8 => {
        8
      }
    };
    ...
    if value_bytes <= 4 || bigtiff && value_bytes <= 8 {
      match self.type_ {
        ...
        Type::UNDEFINED => {
          return Ok(List(self.offset[0..self.count as usize].iter().map(|&b| Byte(b)).collect()));
        }
        Type::SHORT => {
          let mut r = self.r(bo);
          let mut v = Vec::new();
  ...
  fn decode_offset<R, F>(
    &self, value_count: u64, bo: ByteOrder, bigtiff: bool, limits: &super::Limits,
    reader: &mut SmartReader<R>, decode_fn: F,
  ) -> TiffResult<Value>
  where
    R: Read + Seek,
    F: Fn(&mut SmartReader<R>) -> TiffResult<Value>,
  {
    ...
    let offset = if bigtiff { self.r(bo).read_u64()? } else { self.r(bo).read_u32()?.into() };
...
fn offset_to_bytes(n: usize, entry: &Entry) -> TiffResult<Value> {
  Ok(List(entry.offset[0..n].iter().map(|&e| Unsigned(u32::from(e))).collect()))
}

/// Extracts a list of SBYTE tags stored in an offset
#[inline]
fn offset_to_sbytes(n: usize, entry: &Entry) -> TiffResult<Value> {
  Ok(List(entry.offset[0..n].iter().map(|&e| Signed(i32::from(e as i8))).collect()))
}
